...
class HasTSHDProduction(HasVisor, HasJetTSHD, HasSoilTSHD, Log):
    ...
    def situ_production(self, env, velocity_trailing, hopper_capacity):
        ...
        loading_time = hopper_capacity / situ_production
    ...
    # ):
    #     print('post_process: {}'.format(activity.name))

    #     # the power consumption of sailing empty phase is calculated based on the "calculate_power" developed by Stijn Lamers     
    #     if activity.name == "sailing empty":
    #         situ_production = 0
    #         loading_time = 0

    #     # the power consumption of loading phase is calculated based on the functions developed by Diederik Janssen    
    #     elif activity.name == "loading":
    #         situ_production = situ_production
    #         loading_time = loading_time
        
    #     # the power consumption of sailing full phase is calculated based on the "calculate_power" developed by Stijn Lamers
    #     elif activity.name == "sailing filled": 
    #         situ_production = 0
    #         loading_time = 0
        
    #     # the power consumption of dumping phase is calculated based on the functions developed by Diederik Janssen 
    #     elif activity.name == "unloading": 
    #         situ_production = situ_production
    #         loading_time = 0
...
#     def pump_input_func(pump_input):
    
#         pump_input = pump_input_check(pump_input)
#         pump_input["CnstRelTrq"], pump_input["CnstRelRpm"] = calc_pump_characteristics(pump_input)

#         pump_input["rpm_max"] = rounddown(np.max(pump_input["CnstRelRpm"]) * pump_input["rpm_nom"], 1)
#         pump_input["rpm_min"] = roundup(calc_rpmMin(pump_input["rpm_nom"], pump_input["CnstRelRpm"], pump_input["CnstRelTrq"]), 1)
#         pump_input = pump_input_check(pump_input)

#         if pump_input["rpm_act"] >= pump_input["rpm_max"]:
#             pump_input["rpm_act"] = pump_input["rpm_max"] - 0.01

#         if pump_input["rpm_act"] <= pump_input["rpm_min"]:
#             pump_input["rpm_act"] = pump_input["rpm_min"] + 0.01
...
#     def soil_func(Pump_soil, pump_input):
#         soil = Pump_soil
#         soil["sg_mix"] = create_sg_mix(soil["sg_mix"], soil["sg_water"])
#         soil["fc"] = calc_pipeFrictionModel(pump_input, soil)
        
#         return soil


#     def pump_working_point(self, env):
#         'From previous function'
#         pump_input_IBP = {
#             "CnstH": [456.49918325208, -13.4318100659517, -0.430151867867828],
#             "CnstP": [599.725385687135, 359.500309116645, -9.95172192404917],
#             "pipeFrictionModel": 2,
#             "power_nom": 2750,
#             "diam_nom": 2.620,
#             "width_imp": 0.560,
...
#         }
#         pump_input_UWP = {
#             "CnstH": [324.82923076923, -21.7215384615379, -1.1384615384616],
#             "CnstP": [396.182012153847, 220.395147974358, -16.7069939743589],
#             "pipeFrictionModel": 2,
#             "power_nom": 2550,
#             "diam_nom": 1.930,
#             "width_imp": 0.480,
...
#         }
#         pump_input_Jet = {
#             "CnstH": [486.74630564369, -21.078347044683, -5.86506707679301],
#             "CnstP": [346.882762656812, 398.668502056727, -25.0314356085345],
#             "pipeFrictionModel": 1,
#             "power_nom": 2150,
#             "diam_nom": 1.610,
#             "width_imp": 0.340,
...
#         soil = soil, pump_input_IBP #Pump input only used to determine the friction model which is the same for both pumps
#         soil_jet = soil, pump_input_Jet
        
#         'dictionary from jupyter notebook stored in own dict and transfered here:'
#         general = {
#             "g": 9.81, # [m/s2]
#             "temperature": 15, # [Celsius]
#             "H_atmospheric": 101.325, # [kPa] atmospheric pressure (mean sea-level atmospheric pressure on Earth)(source wikipedia)
#         }

#         resistance_input_pump = {
#             "LambdaModel": 2,
#             "hpump": 1.6,  # [m]
#             "d_pipe": 1.2,  # [m]
#             "pipelinelength": 100,  # [m]
#             "ksi": {"additional": 3.0, "water": 1.8, "mixture": 1.0},  # [-]
#             "heightreclamation": 3,  # [m]
#             "Z": 18,  # [m]
#         }

#         resistance_input_jet = {
#             "LambdaModel": 2,
#             "hpump": 1.6,  # [m]
#             "d_pipe": 0.715,  # [m]
#             "pipelinelength": 90,  # [m]
...
#         Q = np.arange(0.01, roundup(calc_pump_Qmax(pump_input_IBP["CnstH"]), 0.005)+1, 0.005)
#         limits = calc_characteristic_engine(pump_input_IBP)
        
#         'Pump characteristics calculation'
#         pump_IBP = {"water": calc_pump_PDL_spinner(Q, pump_input_IBP, soil, soil["sg_water"], additional_info=False)}
#         pump_IBP["mixture"] = calc_pump_PDL_spinner(Q, pump_input_IBP, soil, soil["sg_mix"][-1])
#         pump_UWP = {"water": calc_pump_PDL_spinner(Q, pump_input_UWP, soil, soil["sg_water"], additional_info=False)}
#         pump_UWP["mixture"] = calc_pump_PDL_spinner(Q, pump_input_UWP, soil, soil["sg_mix"][-1])
#         pump_Jet = {"water": calc_pump_PDL_spinner(Q, pump_input_Jet, soil_jet, soil_jet["sg_water"], additional_info=False)}
        
#         pump_combined_water = pd.DataFrame(Q, columns=["Q"])
#         pump_combined_water["H"] = pump_IBP["water"]["H"] + pump_UWP["water"]["H"]
#         pump_combined_water["P"] = pump_IBP["water"]["P"] + pump_UWP["water"]["P"]
        
#         pump_combined_mix = pd.DataFrame(Q, columns=["Q"])
#         pump_combined_mix["H"] = pump_IBP["mixture"]["H"] + pump_UWP["mixture"]["H"]
#         pump_combined_mix["P"] = pump_IBP["mixture"]["P"] + pump_UWP["mixture"]["P"]

#         pump_combined = {"water": pump_combined_water}
#         pump_combined["mixture"] = pump_combined_mix
        
#         'Resistance calculation'
#         pipeline_resistance_pump = {"water": calc_pipeline_resistance_water(Q, resistance_input_pump, general, soil["sg_water"])}
#         pipeline_resistance_pump["mixture"] = calc_pipeline_resistance_mixture(Q, resistance_input_pump, general, soil, soil["sg_mix"][-1])
#         pipeline_resistance_jet = {"water": calc_pipeline_resistance_water(Q, resistance_input_jet, general, soil_jet["sg_water"])}
        
#         working_point = pd.concat(
#             [
#                 calc_workingpoints(pump_combined["water"], pipeline_resistance_pump["water"]),
#                 calc_workingpoints(pump_combined["mixture"], pipeline_resistance_pump["mixture"]),
#                 calc_workingpoints(pump_Jet["water"], pipeline_resistance_jet["water"]),
#                 ], keys= ["Parallel_pumps_ps_water","Parallel_pumps_ps_mixture", "Jet_pump_ps_water"]
#             )
        
#         Pump_power_wat =  working_point.iloc[0]["P"]
#         Pump_power_mix =  working_point.iloc[1]["P"]
#         Pump_power_jet =  working_point.iloc[2]["P"]
...
#                     core.LogState.UNKNOWN,
#                     {
#                         "Pump_power_wat": Pump_power_wat,
#                         "Pump_power_mix": Pump_power_mix,
#                         "Pump_power_jet": Pump_power_jet,
#                         }
#                     )
